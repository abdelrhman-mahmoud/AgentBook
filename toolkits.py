...
import pandas as pd
from find_intervals import find_available_intervals
import re
...
def reschedule_booking(room_id: int, customer_name: str, start_time: str, customer_id: str, 
    ...
    cancel_result = cancel_booking.invoke({'booking_id':booking_id})
    
    if not cancel_result.startswith("Successfully"):
        return f"Error rescheduling: {cancel_result}"
    
    
    book_result = book_room.invoke({'room_id':room_id, 'customer_name':customer_name, 'start_time':start_time, 'customer_id':customer_id, 'duration':duration})
